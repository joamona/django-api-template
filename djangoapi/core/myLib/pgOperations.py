...
class PgConnect(PgConnection):
    ...
    def __init__(self, database: str,user: str,password: str,host: str ,port: Union[str,int]):
        ...
        """
        self.conn=psycopg.connect(dbname=database, user=user, password=password, host=host, port=port)
...
#         # Usamos Identifier para proteger contra inyección SQL en nombres de tablas/esquemas
#         complete_counter_name = sql.Identifier(self.counter_schema, counter_name)

#         # 2. Ejecutar la creación del esquema
#         self.pgo.pgConnection.cursor.execute(sql.SQL("create schema if not exists counters"))
...
#         cons = sql.SQL("create sequence {name} as integer start with {start} increment by {inc}").format(
...
#         # 2. Usamos sql.Literal para que psycopg lo envíe como una cadena: 'counters.c1'

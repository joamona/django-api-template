...
@authentication_classes([]) # Para desactivar la autenticación en esta vista
@permission_classes((permissions.AllowAny,)) # Para desactivar la autenticación en esta vista
...
def customKnoxLogoutView(request):
    token = request.META.get('HTTP_AUTHORIZATION', False)
    if token:
        try:
            token_key = str(token).split()[1].encode("utf-8")
...
@authentication_classes([])
@permission_classes((permissions.AllowAny,))
def logoutAllUserSessionsView(request):
    """
    ...
    if user.username.lower() != posted_username.lower():
        ...
        if not user.groups.filter(groupname='admin').exists():
        ...
        if not user.groups.filter('admin').exists():
            men2="El usuario no pertenece a un grupo con permisos para cerrar las sesiones de otros."
        else:
...
@authentication_classes([])
@permission_classes((permissions.AllowAny,))
def logoutAllUsersSessionsView(request):
    """
